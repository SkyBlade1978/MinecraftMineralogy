...
public class Mineralogy {
	...
	public Mineralogy(FMLJavaModLoadingContext loadingContext) {
		...
		MineralogyConfig.registerRecipeConditions(modBusGroup);
		MineralogyItemGroups.registerRuntimeListeners();

		FMLCommonSetupEvent.getBus(modBusGroup).addListener(this::setup);
		MineralogyFluids.register(modBusGroup);
		ServerAboutToStartEvent.BUS.addListener(Priority.HIGHEST, LegacyWorldDataHook::onServerAboutToStart);
		TagsUpdatedEvent.BUS.addListener(CobblestoneTagPolicy::onTagsUpdated);
	}
